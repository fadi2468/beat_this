...
class BeatTrackingDataset(Dataset):
    ...
    def _load_dataset_item(self, item_name):
        ...
        for aug_filename in precomputed_augmentation_filenames(self.augmentations):
            if (f"{remainder}/{aug_filename[:-4]}") not in self.spects.get(
                ...
                # print(self.spects.get(  dataset, () ))
                # print(f"{remainder}/{aug_filename[:-4]}")
                # print("heeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
    ...
    def _get_spect(self, item):
        try:
            ...
            # Normalize the path to handle Windows-style backslashes
            dataset, filename = str(item["spect_path"]).replace("\\", "/").split("/", 1)
            spect = self.spects[dataset][filename[:-4]]
        except KeyError:
            # Construct the full path using pathlib for better cross-platform support
            spect_path = (self.spect_basepath / item["spect_path"]).resolve()
    ...
    def __getitem__(self, index):
        if isinstance(index, (int, np.int64)):  # when index is a single int
            ...
            print(f"Loaded spectrogram shape: {spect.shape}")
